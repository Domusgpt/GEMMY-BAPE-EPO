...
class UserEventReactiveCore {
    constructor(epoCore) {
        if (!epoCore) throw new Error("EPOCore instance required for UserEventReactiveCore.");
        this.epoCore = epoCore;
        this.isActive = false;
        this.visualizers = [];
    ...
    calculateInteractionLevels() {
        ...
        // Map interaction levels to EPO potentials
        // More movement and precision (deliberate action) increases integrative potential
        const integrativeDelta = (this.analysisData.movementSmooth * 0.001 + this.analysisData.precisionSmooth * 0.002) * deltaTime;
        // More velocity (rapid, less controlled changes) increases dispersive potential
        const dispersiveDelta = (this.analysisData.velocitySmooth * 0.002) * deltaTime;

        this.epoCore.influence(integrativeDelta, dispersiveDelta);
    }
