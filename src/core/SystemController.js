...
class SystemController {
    constructor(epoNarrativeManager, epoCore) {
        ...
        this.userEventReactive = new UserEventReactiveCore(epoCore); // Pass epoCore to UserEventReactiveCore
        ...
        this.epoAgentController = null;
        this.epoNarrativeManager = epoNarrativeManager; // Store the narrative manager
        this.epoNarrativeVisualizer = null;
        this.epoCore = epoCore; // Store the EPOCore instance
    ...
    async initialize() {
        ...
        try {
            ...
            // Phase 10: Start EPO Agent Multi-Mode
            console.log('🤖 Phase 10: Starting EPO Agent Multi-Mode...');
            this.startEPOAgentMultiMode();
    ...
    async initializeVisualizers() {
        ...
        // Initialize EPONarrativeVisualizer
        const narrativeCanvas = document.createElement('canvas'); // Create a dedicated canvas for the narrative
        narrativeCanvas.id = 'narrative-canvas';
        narrativeCanvas.style.position = 'absolute';
        narrativeCanvas.style.top = '0';
        narrativeCanvas.style.left = '0';
        narrativeCanvas.style.width = '100%';
        narrativeCanvas.style.height = '100%';
        narrativeCanvas.style.pointerEvents = 'none'; // Allow clicks to pass through
        document.body.appendChild(narrativeCanvas);

        try {
            const gl = narrativeCanvas.getContext('webgl', { antialias: true });
            const shaderManager = new ShaderManager(gl, null, null); // Narrative visualizer manages its own simple shaders
            this.epoNarrativeVisualizer = new EPONarrativeVisualizer(gl, shaderManager);
            this.visualizers.set('epo-narrative-visualizer', this.epoNarrativeVisualizer);
            console.log('📖 Initialized EPO Narrative Visualizer');
        } catch (error) {
            console.error('❌ EPO Narrative Visualizer failed:', error);
        }

        // Initialize AIPersonalityVisualizer
        try {
            // AIPersonalityVisualizer doesn't need a dedicated canvas, it sets global uniforms
            // It needs a reference to the main shaderManager used by other visualizers
            const mainShaderManager = this.visualizers.get('board-visualizer').shaderManager; // Assuming board visualizer has the main shader manager
            this.aiPersonalityVisualizer = new AIPersonalityVisualizer(null, mainShaderManager); // Pass null for gl as it doesn't draw directly
            this.visualizers.set('ai-personality-visualizer', this.aiPersonalityVisualizer);
            console.log('🧠 Initialized AI Personality Visualizer');
        } catch (error) {
            console.error('❌ AI Personality Visualizer failed:', error);
        }
        ...
        const holographicVisualizer = new HolographicVisualizer(canvasId, 'content', 1.0);
        this.visualizers.set(`holographic-${canvasId}`, holographicVisualizer);
    ...
    async setupInteractionSystem() {
        ...
        this.setupNeoskeuomorphicInteractions();
    ...
    decayScrollAccumulation() {
        ...
        if (Math.abs(this.scrollAccumulation) < 0.1) {
            ...
            this.visualizers.forEach(viz => {
                if (viz instanceof HolographicVisualizer) {
                    viz.updateChaos(0);
                }
            });
    ...
    setupNeoskeuomorphicInteractions() {
        document.addEventListener('mousemove', (e) => {
    ...
    startReactiveUpdateLoop() {
        ...
        const reactiveUpdateLoop = (currentTime) => {
            ...
            interactionData.deltaTime = deltaTime; // Add deltaTime to interactionData
            ...
            // Draw narrative visualizer
            if (this.epoNarrativeVisualizer) {
                this.epoNarrativeVisualizer.draw();
            }
    ...
    updateVisualizersWithReactiveParams(effectiveParams, interactionData, currentState) {
        ...
        // Update EPOCore with deltaTime
        this.epoCore.update(interactionData.deltaTime); // Assuming interactionData has deltaTime
        const epoPotentials = this.epoCore.getPotentials();

        // Determine narrative theme and apply to visualizers
        let currentNarrativeTheme = 'default';
        const currentSection = this.epoNarrativeManager.getCurrentSection();
        if (currentSection) {
            currentNarrativeTheme = this.epoNarrativeManager.getSectionTheme(currentSection.content);
        }

        // Update EPO-specific visualizers
        const integrativeViz = this.visualizers.get('epo-integrative-visualizer'); // Assuming ID
        if (integrativeViz && typeof integrativeViz.update === 'function') {
            integrativeViz.update(interactionData.deltaTime, epoPotentials.integrative, epoPotentials.scaleFactor, currentNarrativeTheme);
        }

        const dispersiveViz = this.visualizers.get('epo-dispersive-visualizer'); // Assuming ID
        if (dispersiveViz && typeof dispersiveViz.update === 'function') {
            dispersiveViz.update(interactionData.deltaTime, epoPotentials.dispersive, epoPotentials.scaleFactor, currentNarrativeTheme);
        }

        // Update narrative visualizer
        if (this.epoNarrativeVisualizer && this.epoNarrativeManager) {
            if (currentSection) {
                this.epoNarrativeVisualizer.updateText(currentSection.content);
                this.epoNarrativeVisualizer.update(interactionData.deltaTime, epoPotentials, currentNarrativeTheme);
            }
        }
        
        // Update AIPersonalityVisualizer based on narrative theme
        const aiPersonalityViz = this.visualizers.get('ai-personality-visualizer'); // Assuming ID
        if (aiPersonalityViz && typeof aiPersonalityViz.setPersonality === 'function') {
            // Map narrative themes to AI personalities for visual effect
            let personalityType = 'none';
            if (currentNarrativeTheme === 'integrative') {
                personalityType = 'structural-consciousness';
            } else if (currentNarrativeTheme === 'dispersive') {
                personalityType = 'biological-cognitive-consciousness';
            } else if (currentNarrativeTheme === 'consciousness') {
                personalityType = 'biological-cognitive-consciousness';
            } else if (currentNarrativeTheme === 'quantum') {
                personalityType = 'proto-consciousness';
            }
            aiPersonalityViz.setPersonality(personalityType);
        }
    ...
    startEPOAgentMultiMode() {
        this.epoAgentController = new EPOAgentController(this.visualizers);
        this.epoAgentController.initialize(6); // Create 6 agents, one for each card

        const agentUpdateLoop = () => {
            this.epoAgentController.update();
            this.epoAgentController.render();
            requestAnimationFrame(agentUpdateLoop);
        };
